...
class Aliens:
    ...
    def update(self):
        ...
        if pg.sprite.spritecollideany(self.game.ship, self.alien_group):
            self.game.reset()
...
class Alien(Sprite):   # INHERITS from SPRITE
    ...
    def killed(self):
        if self.dead and not self.timer_switched:
            if self.number == 0:
                self.game.stats.score += self.settings.alien_points0
            if self.number == 1:
                self.game.stats.score += self.settings.alien_points1
            if self.number == 2:
                self.game.stats.score += self.settings.alien_points2
